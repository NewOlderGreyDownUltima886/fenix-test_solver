...
def make_a_request(url, data, silens=True, streaming=False, num_of_error=0, auth_mode=False, get_request=False, multipart_form_data=False):
    ...
    try:
        if get_request == True:
            ...
            if multipart_form_data == False:
                ...
                a = requests.post(url, data=data, headers=headers2, cookies=get_cookies(), stream=streaming) #Для POST-запросов b
    except Exception as E:
        print(f'''\n----------------------------ERROR-------------------------------------------''')
        ...
        if len(arr) > 0:
            for i in arr:
                ...
                if "Вернуться в текущий семестр" in str(i.text):
                    return True
        return load_semestr(num_of_sem, num_of_try=num_of_try)
