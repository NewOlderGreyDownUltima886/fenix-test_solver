...
def get_new_session_id():
    ...
    if "ASP.NET_SessionId" in set_cookies:
        start = set_cookies.find("ASP.NET_SessionId") + 18
...
4. Проверить обновление (в разработке)
0. Выйти
""")
        ...
        if chose == "1":
            if get_login() and get_pass():
                if auth():
                    ...
                    print("\nКажется в данных ошибка! Попробуйте еще раз:")
                    login = input("Логин: ")
                    passw = input("Пароль: ")
        ...
        if auth_bool == True:
            while True:
                ...
                a = user_input(f"Решаю {vsego_testov} тестов по предмету \"{name_pred_r}\" — с {num_testing_r+1} по {do_kakogo_testa_vkluchitelno_r+1} включительно, правильно?\n(y/n): ")
                if ("y" in a): break
                elif ("Y" in a): break
                elif ("н" in a): break
                elif ("Н" in a): break
            ...
            while num_testing <= do_kakogo_testa_vkluchitelno:
                ...
                if proverka == True:
                    ...
                    print(f"\n---------------------------------------------------------------\n{datetime.datetime.now().strftime("%H:%M:%S")}:——————————>1.ЗАХОЖУ НА ТЕСТ {num_testing+1} \"{name_of_test}\" ({num_non_smisl_of_test}), время: {time_start_test.strftime("%H:%M:%S")}")
                    
                    select_question("1")
                    if answers_complete == False:
                        print(f"{datetime.datetime.now().strftime("%H:%M:%S")}:——————————>2.ПРОБЕГАЮСЬ ПО ВОПРОСАМ...")
                    ...
                    num_of_mistakes = random.randint(1, 4)
                    ...
                    #готовися ждать
                    time_to_wait = random.randint(180, 500) 
