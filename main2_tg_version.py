...
class FENIX:
    ...
    def print_tests_of_predmet_by_its_index_and_chose_test_step2(self, num_of_pred):
        ...
        #3 - Cпрашиваем с какого теста начать решать и по какой
        '''try:
            num_of_test = int(self.user_input("\nC какого теста начать (по счёту): ")) - 1
            if num_of_test == -1:
                return -1
            if (num_of_test > (len(test_name_list)-1)) or (num_of_test < 0):
                print("hui3")
                return False
            do_kakogo_testa_vkluchitelno = int(self.user_input("До какого теста решать включительно: ")) - 1 #до какого теста  (начиная с нуля)
...
def start_func(message):
    ...
    key_new = make_keyboard([['1. Решить тесты', 0],
                             [f'2. Изменить ошибки (от {fenix.num_of_mistakes_ot} до {fenix.num_of_mistakes_do})', 1], 
                             [f'3. Изменить время ожидания (от {fenix.time_to_wait_ot} до {fenix.time_to_wait_do} секунд', 2], 
...
def change_error(call):
    print("\n----------------------------------")
    print("Введите диапозон ошибок через тире (пример 1-4):")
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Введите диапозон ошибок через тире (пример 1-4):", reply_markup=make_keyboard([['Отменить ввод❌', -1]]))  
...
def change_error_next_step(message, call):
    ...
    sms = bot.send_message(message.from_user.id, text=f"✅Установлено количество ошибок от {fenix.num_of_mistakes_ot} до {fenix.num_of_mistakes_do}\nСообщения будут удалены через 2...")
    print('''\n--—-----------------------------------------''')
    name = str(message.from_user.username) + "(" +str(message.from_user.id) +", " + str(message.chat.id) +")"
    print(f"\nПОЛУЧИЛ СМС от {name} () (УДАЛИТЬ ЧЕРЕЗ 2 СЕКУНДЫ):", message.text)
    x = 2
    time.sleep(1)
    while x != 1:
      x = x - 1
      bot.edit_message_text(chat_id=message.chat.id, message_id=sms.id, text=f"✅Установлено количество ошибок от {fenix.num_of_mistakes_ot} до {fenix.num_of_mistakes_do}\nСообщения будут удалены через {x}...")
      time.sleep(1)
    if del_msg(message.chat.id, [sms.id, message.message_id]):
      print(f" ——> УДАЛЕНЫ НЕНУЖНЫЕ СООБЩЕНИЯ ОТ {name}:  [{sms.id}, {message.message_id}]")
    else:
      print(f" ——> ERROR: НЕ ПОЛУЧИЛОСЬ УДАЛИТЬ НЕНУЖНЫЕ СООБЩЕНИЯ ОТ {name}:  [{sms.id}, {message.message_id}]")
    try:
...
def change_time_interval(call):  
    print("\n----------------------------------")
    print("Введите диапозон ожидаемого времени через тире в секундах(пример 300-350):")
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Введите диапозон ожидаемого времени через тире в секундах(пример 300-350):", reply_markup=make_keyboard([['Отменить ввод❌', -1]]))  
...
def change_time_interval_next_step(message, call):
    ...
    sms = bot.send_message(message.from_user.id, text=f"✅Установлено время ожидания от {fenix.time_to_wait_ot} до {fenix.time_to_wait_do} секунд\nСообщения будут удалены через 2...")
    print('''\n--—-----------------------------------------''')
    name = str(message.from_user.username) + "(" +str(message.from_user.id) +", " + str(message.chat.id) +")"
    print(f"\nПОЛУЧИЛ СМС от {name} () (УДАЛИТЬ ЧЕРЕЗ 2 СЕКУНДЫ):", message.text)
    x = 2
    ...
    while x != 1:
      x = x - 1
      bot.edit_message_text(chat_id=message.chat.id, message_id=sms.id, text=f"✅Установлено время ожидания от {fenix.time_to_wait_ot} до {fenix.time_to_wait_do} секунд\nСообщения будут удалены через {x}...")
      time.sleep(1)
    if del_msg(message.chat.id, [sms.id, message.message_id]):
...
def enter_new_data_LOGIN(call):
    print("iamhere6")
    fenix.array_to_del = []
    to_print = f"Ваши данные\nЛогин: {fenix.get_login()}\nПароль: {fenix.get_pass()}"
...
def reakcia_na_no_name_sms_from_user(message):
  ...
  name = str(message.from_user.username) + "(" +str(message.from_user.id) +", " + str(message.chat.id) +")"
  print(f"\nПОЛУЧИЛ СМС от {name} () (УДАЛИТЬ ЧЕРЕЗ 2 СЕКУНДЫ):", message.text)
  sms = bot.send_message(message.from_user.id, text="Пожалуйста, пользуйтесь кнопками!\n(сообщения будут удалены через 2)" )#+ str(x) + " секунды)")
  x = 2
  time.sleep(1)
  while x != 1:
    x = x - 1
